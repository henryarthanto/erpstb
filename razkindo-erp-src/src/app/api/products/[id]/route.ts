...
export async function GET(
  ...
  try {
    ...
    // Check cache first for single product
    const canSeeHppCacheKey = `canSeeHpp:${authUserId}`;
    let canSeeHppCached = await cacheGet<boolean>(canSeeHppCacheKey);
    let canSeeHpp: boolean;
    if (canSeeHppCached !== null) {
      canSeeHpp = canSeeHppCached;
    } else {
      const { data: authUser } = await db.from('users').select('role').eq('id', authUserId).single();
      canSeeHpp = !!(authUser && authUser.role && ['super_admin', 'keuangan'].includes(authUser.role));
      await cacheSet(canSeeHppCacheKey, canSeeHpp, { ttlMs: 300_000 }); // 5 min
    }

    const productCacheKey = `api:product:${id}:${canSeeHpp}`;
    try {
      const cached = await cacheGet<any>(productCacheKey);
      if (cached) {
    ...
    try {
      await cacheSet(productCacheKey, productCamel, { ttlMs: 60_000 }); // 1 min
    } catch { /* non-fatal */ }

    return NextResponse.json({ product: productCamel });
  } catch (error: any) {
...
export async function DELETE(
  ...
  try {
    ...
    // Invalidate all product caches on delete
    try { await invalidateAllProductCaches(); } catch { /* non-fatal */ }
