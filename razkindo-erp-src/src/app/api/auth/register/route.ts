...
export async function POST(request: NextRequest) {
  try {
    ...
    // SECURITY: Block additional super_admin registration if one already exists.
    // The FIRST super_admin registration is always allowed (initial setup).
    if (validatedRole?.toLowerCase() === 'super_admin') {
    ...
    try {
      // Status is determined by role (super_admin = auto-approved, others = pending if super_admin exists)
