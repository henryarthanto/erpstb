...
export default function BankMutationsTab({ bankAccounts }: BankMutationsTabProps) {
  ...
  const handleMutationClick = (mutation: MootaMutation) => {
    ...
    if (isCredit) {
      // Uang masuk — show ALL unpaid/partial transactions (no amount filtering)
      // User said: no need to match amount because customers sometimes get admin deductions
      setShowActionPopup(true);
      setMatchedTransactions([]); // clear while loading
      apiFetch<{ transactions: any[] }>(
        `/api/transactions?type=sale&status=approved&limit=200`
      ).then((res) => {
        // Map API response (nested objects) to flat MatchedTransaction format
        const unpaid = (res.transactions || [])
          ...
          .map((tx: any) => ({
            ...
            remainingAmount: Number(tx.remainingAmount) || 0,
            paymentStatus: tx.paymentStatus || 'unpaid',
          }));
        setMatchedTransactions(unpaid);
      }).catch((err) => {
        console.error('[BankMutationsTab] Failed to fetch transactions:', err);
        toast.error('Gagal memuat invoice — coba lagi');
  ...
  // ─── Sync Moota Balance to Internal Bank Accounts ──────────────

  const syncMootaBalance = async () => {
    ...
    try {
      ...
      }>('/api/finance/moota/sync-balance', {
        method: 'POST',
        body: JSON.stringify({ bankId: selectedBankId || undefined }),
      });

      if (res.success && res.synced > 0) {
        toast.success(`${res.message} ${res.results.filter(r => r.matched).map(r => `${r.accountNo}: ${formatCurrency(r.newBalance)}`).join(', ')}`);
        // Refresh bank accounts list in parent
        queryClient.invalidateQueries({ queryKey: ['bank-accounts'] });
        queryClient.invalidateQueries({ queryKey: ['finance-pools'] });
      } else if (res.unmatched && res.unmatched > 0) {
        toast.warning(res.unmatchedMessage || 'Tidak ada rekening yang cocok. Pastikan nomor rekening di Settings sama dengan Moota.');
      } else {
        toast.info('Tidak ada perubahan saldo.');
      }
    } catch (err: any) {
      toast.error(err?.message || 'Gagal sinkronisasi saldo');
    } finally {
              ...
              {selectedBankId && (
                <>
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">Sync Mutasi</span>
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    className="h-7 text-xs gap-1 border-blue-200 text-blue-700"
                    onClick={syncMootaBalance}
                    disabled={syncingBalance}
                  >
                    <Database className={`w-3.5 h-3.5 ${syncingBalance ? 'animate-pulse' : ''}`} />
                    <span className="hidden sm:inline">Sync Saldo</span>
                  </Button>
                </>
              )}
            </div>
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {matchedTransactions.length > 0
                      ? `${matchedTransactions.length} invoice belum lunas ditemukan`
                      : 'Memuat invoice...'}
                  </p>
                </div>
              </button>
      ...
      <Dialog open={showLunasDialog} onOpenChange={setShowLunasDialog}>
        <DialogContent className="sm:max-w-lg w-[calc(100%-2rem)] max-h-[80dvh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Pilih invoice yang belum lunas untuk ditandai sebagai pembayaran dari mutasi bank
            </DialogDescription>
          </DialogHeader>
          ...
          {matchedTransactions.length > 0 && (
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Cari nama usaha / invoice..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-input bg-background focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-500"
              />
            </div>
          )}
          ...
          {matchedTransactions.length === 0 ? (
            <div className="py-8 text-center text-muted-foreground">
              ...
              <p className="text-sm">Tidak ada invoice yang belum lunas</p>
              <p className="text-xs mt-1">Semua invoice sudah terbayar</p>
            </div>
          ) : (() => {
            const filtered = matchedTransactions.filter((tx) => {
                ...
                {filtered.map((tx) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                      <span className="font-medium text-foreground/80">{tx.customerName} — {tx.unitName}</span>
                      <span className="font-medium text-green-700 dark:text-green-400">Sisa: {formatCurrency(tx.remainingAmount)}</span>
                    </div>
