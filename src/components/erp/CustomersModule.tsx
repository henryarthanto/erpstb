...
function CustomerForm({ unitId, units, onSuccess, customer, salesUserId }: {
  ...
  const [formData, setFormData] = useState({
    ...
    cashbackEnabled: (customer as any)?.cashbackValue > 0 ? true : false,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 max-sm:space-y-2.5">
      {needsUnitSelection && (
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <Switch
            ...
            onCheckedChange={(checked) => setFormData({ ...formData, cashbackEnabled: checked, cashbackValue: checked ? formData.cashbackValue || '0' : '0' })}
          />
        </div>
      ...
      <DialogFooter className="max-sm:sticky max-sm:bottom-0 max-sm:bg-background max-sm:pt-2 max-sm:border-t max-sm:z-10">
    ...
    <div className="space-y-4 overflow-x-hidden">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          ...
          <DialogContent className="sm:max-w-md max-sm:top-auto max-sm:bottom-0 max-sm:left-0 max-sm:right-0 max-sm:translate-x-0 max-sm:translate-y-0 max-sm:w-full max-sm:max-w-full max-sm:max-h-[85dvh] max-sm:flex max-sm:flex-col max-sm:p-0 max-sm:gap-0 max-sm:rounded-b-none max-sm:rounded-t-2xl">
            <DialogHeader className="max-sm:px-4 max-sm:pt-4 max-sm:pb-0 max-sm:shrink-0">
              <DialogTitle className="max-sm:text-base">Tambah Pelanggan</DialogTitle>
            </DialogHeader>
            <div className="max-sm:flex-1 max-sm:min-h-0 max-sm:overflow-y-auto max-sm:overscroll-contain max-sm:px-4 max-sm:py-3">
              <CustomerForm
                unitId={unitId || ''}
                units={units}
      ...
      <Dialog open={!!editingCustomer} onOpenChange={(open) => { if (!open) setEditingCustomer(null); }}>
        <DialogContent className="sm:max-w-md max-sm:top-auto max-sm:bottom-0 max-sm:left-0 max-sm:right-0 max-sm:translate-x-0 max-sm:translate-y-0 max-sm:w-full max-sm:max-w-full max-sm:max-h-[85dvh] max-sm:flex max-sm:flex-col max-sm:p-0 max-sm:gap-0 max-sm:rounded-b-none max-sm:rounded-t-2xl">
          <DialogHeader className="max-sm:px-4 max-sm:pt-4 max-sm:pb-0 max-sm:shrink-0">
            <DialogTitle className="max-sm:text-base">Edit Pelanggan</DialogTitle>
          </DialogHeader>
          <div className="max-sm:flex-1 max-sm:min-h-0 max-sm:overflow-y-auto max-sm:overscroll-contain max-sm:px-4 max-sm:py-3">
            {editingCustomer && (
              <CustomerForm
