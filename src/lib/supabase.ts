...
//
// CRITICAL: PrismaClient is NOT created at module evaluation time.
// Instead, it's lazily initialized on first access via getPrisma().
// On first call, we read .env directly to ensure DATABASE_URL is available
// (Next.js instrumentation may run before .env is auto-loaded).
// ─────────────────────────────────────────────────────────────────────
...
/**
 * Get or create the PrismaClient singleton (lazy initialization).
 * Safe to call from API routes, instrumentation, or any server code.
 */
function getPrisma(): PrismaClient {
  ...
  if (!dbUrl.startsWith('postgresql://') && !dbUrl.startsWith('postgres://')) {
    console.error('[Supabase] DATABASE_URL is missing or invalid. Check your .env file.');
  }

  const client = new PrismaClient({
    datasourceUrl: buildPooledUrl(dbUrl),
...
/** Prisma Client — lazily initialized singleton */
...
/** Replace undefined values with null (recursive). Prisma rejects undefined fields
 *  but accepts null for optional scalar FK fields (e.g. unitId, customerId).
 *  Replacing with null instead of removing the key preserves Prisma's XOR type
 *  detection between CreateInput (relation objects) and UncheckedCreateInput (scalar IDs). */
function nullifyUndefined(obj: any): any {
  if (obj === null || obj === undefined) return null;
  if (Array.isArray(obj)) return obj.map(nullifyUndefined);
  if (typeof obj === 'object' && !(obj instanceof Date) && !isDecimalLike(obj)) {
    const result: Record<string, any> = {};
    for (const [key, value] of Object.entries(obj)) {
      result[key] = nullifyUndefined(value);
    }
    return result;
  }
  return obj;
...
class PostgrestQueryBuilder {
  ...
  private async executeInsert(): Promise<PostgrestResult> {
    ...
    if (dataArray.length === 1) {
      // Single insert
      const camelData = nullifyUndefined(toCamelCaseDeep(dataArray[0]));
      const result = await model.create({ data: camelData, ...createOptions });
    ...
    if (dataArray.length > 1 && !this.selectFields && !this.includeConfig) {
      ...
      const camelDataArray = dataArray.map(d => nullifyUndefined(toCamelCaseDeep(d)));
    ...
    for (const item of dataArray) {
      const camelData = nullifyUndefined(toCamelCaseDeep(item));
      const result = await model.create({ data: camelData, ...createOptions });
      results.push(result);
    }
  ...
  private async executeUpdate(): Promise<PostgrestResult> {
    ...
    if (idFilter && (this.singleMode || (this.selectFields && Object.keys(this.selectFields).length > 0))) {
      ...
      // Record exists — perform the update by the record's actual id
      // CRITICAL: use existing.id (the real UUID), NOT idFilter.value (which may be a 'key' string, not an id)
      const result = await model.update({
