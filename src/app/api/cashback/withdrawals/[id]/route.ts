  ...
  try {
    ...
    if (data.status === 'processed') {
      ...
      try {
        ...
        if (poolErr instanceof Error && poolErr.message.includes('Insufficient')) {
        ...
        return NextResponse.json(
          { error: `Gagal mengurangi saldo pool: ${poolErr?.message || 'Terjadi kesalahan'}` },
          { status: 500 }
        );
      }
      ...
      try {
        ...
        if (destErr instanceof Error && destErr.message.includes('Insufficient')) {
          return NextResponse.json(
        ...
        return NextResponse.json(
          { error: `Gagal mengurangi saldo ${destLabel}: ${destErr?.message || 'Terjadi kesalahan'} (pool sudah dikembalikan)` },
          { status: 500 }
        );
      }
