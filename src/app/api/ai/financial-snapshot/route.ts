...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      // ── 4. TRANSACTION ITEMS (untuk top products) ──
      // NOTE: transaction_items has NO created_at column — filter by parent transaction date in JS
      db
    ...
    // Filter hanya item dari transaksi penjualan 90 hari terakhir
    // (transaction_items has no created_at, so filter by parent transaction date)
    const allItems = transactionItemsResult.data || [];
    const saleItems = allItems.filter((item: any) => {
      const tx = item.transaction;
      if (!tx) return false;
      if (tx.type !== 'sale' || (tx.status !== 'approved' && tx.status !== 'paid')) return false;
      // Filter by transaction_date (90 days ago)
      if (tx.transaction_date && new Date(tx.transaction_date) < ninetyDaysAgo) return false;
      return true;
    });
    ...
    const customerPatterns = Array.from(customerMap.values())
      ...
      .filter((c) => c.isActive === true) // Hanya customer aktif
