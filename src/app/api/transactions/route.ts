...
// Sparse column select for the list view — fetches only needed fields instead of select(*)
const LIST_SELECT = `
  id, invoice_no, type, status, payment_status, transaction_date, total, paid_amount, remaining_amount,
  total_hpp, total_profit, hpp_paid, profit_paid, hpp_unpaid, profit_unpaid,
  payment_method, due_date, delivered_at, courier_commission, delivery_distance, version,
  notes, delivery_address, created_at, unit_id, created_by_id, customer_id, supplier_id, courier_id,
  unit:units(id, name),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheParams = { unitId, customerId, type, status, startDate, endDate, limit, authUserRole, createdById: authUserRole === 'sales' ? authUserId : searchParams.get('createdById') };
    const cacheKey = `api:transactions:${JSON.stringify(cacheParams)}`;
    ...
    // Build query — NOTE: supplier join removed for perf; added separately for purchase tx only
    let query = db
      .from('transactions')
      .select(LIST_SELECT);
    ...
    const transactionsCamel = (transactions || []).map((t: any) => {
      ...
      return {
        ...
        supplier: null, // Will be enriched for purchase tx below
    ...
    // Enrich purchase-type transactions with supplier data (only fetch for purchases)
    const purchaseTxIds = transactionsCamel.filter((t: any) => t.type === 'purchase' && t.supplierId).map((t: any) => t.id);
    if (purchaseTxIds.length > 0) {
      try {
        const { data: supplierData } = await db
          .from('transactions')
          .select('id, supplier:suppliers(id, name, phone, address)')
          .in('id', purchaseTxIds);
        const supplierMap = new Map((supplierData || []).map((s: any) => [s.id, s.supplier]));
        for (const tx of transactionsCamel as any[]) {
          if (tx.type === 'purchase') {
            tx.supplier = supplierMap.get(tx.id) || null;
          }
        }
      } catch {
        // Supplier enrichment failure is non-fatal
      }
    }
  ...
  try {
    ...
    if (idempotencyKey) {
      ...
      const { data: existingTx } = await db
        .from('logs')
        .select('entityId, payload')
        .eq('action', 'transaction_created_idempotent')
        .eq('entityId', idempotencyKey)
        .maybeSingle();
    ...
    // Invalidate transactions cache on create
    try {
      await cacheInvalidatePrefix('api:transactions:');
    } catch {
