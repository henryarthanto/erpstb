...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check cache first
    const cacheKey = `api:cashflow:${startDate || 'none'}:${endDate || 'none'}:${typeFilter}:${category}:${page}:${limit}`;
    try {
    ...
    const [
      ...
      // 3. EXPENSES — Standalone expenses (from mutasi bank before fix, direct expense creation)
      fetchExpenses(dateFilter),
      // 4. COURIER HANDOVERS — Internal movement (courier → brankas)
      fetchCourierHandovers(dateFilter),
      // 5. FUND TRANSFERS — Between bank/cashbox
      fetchFundTransfers(dateFilter),
      // 6. CASHBACK WITHDRAWALS — Money OUT to customers
      ...
      // + standalone expenses table (from mutasi bank)
      ...
      Promise.all([
        ...
        (() => {
          let q = db.from('expenses')
            ...
            .not('description', 'ilike', '%dari mutasi bank%'); // Exclude old Moota duplicates already counted in finance_requests
          if (dateFilter.start) q = q.gte('expense_date', dateFilter.start);
          if (dateFilter.end) q = q.lte('expense_date', dateFilter.end + 'T23:59:59');
          return q.then(({ data }) => (data || []).reduce((sum: number, e: any) => sum + (Number(e.amount) || 0), 0));
        })(),
        (() => {
      ...
      ]).then(([reqTotal, expTotal, cbTotal]) => reqTotal + expTotal + cbTotal),
    ...
    // Deduplicate: filter out legacy Moota-matched expenses that are duplicates of finance_requests
    // (before fix, Moota match created BOTH expense + finance_request records, causing double entries)
    const mootaRequestSignatures = new Set(
      processedRequestsResult
        .filter(e => e.description?.includes('dari mutasi bank'))
        .map(e => `${e.amount}:${Math.floor(new Date(e.date).getTime() / 86400000)}`)
    );

    const allEntries: CashFlowEntry[] = [
      ...paymentsResult,
      ...processedRequestsResult,
      ...expensesResult.filter(e => {
        if (!e.description?.includes('dari mutasi bank')) return true;
        const sig = `${e.amount}:${Math.floor(new Date(e.date).getTime() / 86400000)}`;
        return !mootaRequestSignatures.has(sig);
      }),
      ...courierHandoversResult,
      ...fundTransfersResult,
      ...cashbackWithdrawalsResult,
    ...
    try {
      await cacheSet(cacheKey, responseData, { ttlMs: 30_000 });
    } catch {
...
async function fetchExpenses(dateFilter: Record<string, string>): Promise<CashFlowEntry[]> {
  let query = db
    .from('expenses')
    .select(`
      id,
  ...
  if (dateFilter.start) query = query.gte('expense_date', dateFilter.start);
  if (dateFilter.end) query = query.lte('expense_date', dateFilter.end + 'T23:59:59');

  const { data } = await query;
  if (!data) return [];

  return (data || []).map((e: any) => {
    const bankName = (e.bank_account as any)
      ? `Bank: ${(e.bank_account as any).name} (${(e.bank_account as any).bank_name})`
      : '-';
    const createdBy = (e.created_by as any)?.name || 'Sistem';
    ...
    return {
      ...
      metadata: {
        expenseId: e.id,
        bankAccountId: (e.bank_account as any)?.id,
      },
    };
  });
}
