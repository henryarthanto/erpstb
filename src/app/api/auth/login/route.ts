...
export async function POST(request: NextRequest) {
  try {
    ...
    // Detect database connection issues — give clear actionable error
    const dbErrorKeywords = ['DATABASE_URL', 'connect', 'ECONNREFUSED', 'ECONNRESET',
      'ENOTFOUND', 'authentication failed', 'prisma', 'Prisma', 'password authentication',
      'too many connections', 'connection refused', 'timeout', 'P1001', 'P1003'];
    const isDbError = dbErrorKeywords.some(kw => msg.toLowerCase().includes(kw.toLowerCase()));

    if (isDbError) {
      console.error('[Login] DATABASE CONNECTION FAILED — check .env file');
      return NextResponse.json({
        error: 'Koneksi database gagal. Pastikan file .env sudah dikonfigurasi dengan benar (DATABASE_URL, Supabase keys). Jalankan: bash deploy/deploy.sh --setup-env',
        detail: msg,
      }, { status: 503 });
    }
