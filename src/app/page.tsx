...
import { ShimmerSkeleton } from '@/components/native-feel';
...
function AppContent() {
  ...
  // Force portrait orientation on mobile PWA (browsers may ignore manifest orientation)
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const isMobilePWA = /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent)
      && !/STB|Android TV|FireTV|Chromecast/i.test(navigator.userAgent)
      && window.matchMedia('(display-mode: standalone)').matches;
    if (isMobilePWA && screen.orientation?.lock) {
      screen.orientation.lock('portrait-primary').catch(() => {
        // lock may fail on some browsers — non-critical
      });
    }
  }, []);
  ...
  // Wait for hydration + client mount — show app shell skeleton to avoid hydration mismatch
