...
import { cacheGet, cacheSet, invalidateAllProductCaches } from '@/lib/redis-cache';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // ── CACHE: Single raw key, apply unit/HPP filters in-memory ──
    // One cache key for all users regardless of unitId or role.
    // Unit filtering + HPP stripping are applied in-memory after cache read.
    const RAW_CACHE_KEY = 'api:products:raw';
    let productsCamel: any[];
    let cacheHit = false;

    try {
      const cachedRaw = await cacheGet<any[]>(RAW_CACHE_KEY);
      if (cachedRaw) {
        productsCamel = cachedRaw;
        cacheHit = true;
      }
    } catch {
    ...
    if (!cacheHit) {
      ...
      // Cache the RAW products (before unit filter / HPP strip) for reuse
      try {
        await cacheSet(RAW_CACHE_KEY, productsCamel, { ttlMs: 120_000 }); // 2 minutes
      } catch {
        // Cache write failure is non-fatal
      }
    }

    // ── IN-MEMORY FILTERS: Unit enrichment + HPP stripping ──
    ...
    console.log(`[Products:GET] Served ${enrichedProducts.length} products in ${Date.now() - t0}ms (source: ${cacheHit ? 'cache' : 'db'})`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Invalidate all product caches on create
