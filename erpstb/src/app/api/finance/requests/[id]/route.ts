...
async function updateGoodsStatus(existingRequest: any, data: any) {
  ...
  if (newStatus === 'received' && previousStatus !== 'received' && existingRequest.type === 'purchase') {
    ...
    const incrementedProductIds: string[] = [];
    ...
    try {
      for (const item of items) {
        ...
        incrementedProductIds.push(item.productId);
      }
    } catch (stockErr) {
      // Rollback: decrement stock for all items that were successfully incremented
      console.error('[updateGoodsStatus] Stock increment failed, rolling back:', stockErr);
      for (const item of items) {
        if (!incrementedProductIds.includes(item.productId)) continue;
        const stockQty = item.qtyInSubUnit ?? item.qty;
        try {
          await db.rpc('decrement_stock', { p_product_id: item.productId, p_qty: stockQty });
        } catch (rollbackErr) {
          console.error('[updateGoodsStatus] Rollback failed for product', item.productId, rollbackErr);
        }
      }
      throw stockErr;
    }
